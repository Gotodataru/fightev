...
/**
 * Fighter photographs come from ufc.com and are not ours to republish, so the public
 * build ships without them and every fighter falls back to the nameplate the interface
 * already draws for anyone missing a photo. vite.config.ts also strips the files from
 * the output, so a public build cannot ship them even by accident.
 * `VITE_PHOTOS=1 npm run build` puts them back for a local build — that is how the
 * case study's screenshots are captured.
 */
export const PHOTOS_PUBLISHED = import.meta.env.VITE_PHOTOS === '1'

export const photoUrl = (slug: string, avatar = false): string | null =>
  PHOTOS_PUBLISHED ? `${import.meta.env.BASE_URL}data/photos/${slug}${avatar ? '_a' : ''}.webp` : null
