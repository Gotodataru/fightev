...
/** Scroll parallax: returns a CSS translateY that moves `factor` of the scroll distance. */
export function useParallax(factor = 0.12, enabled = true) {
  ...
  useEffect(() => {
    ...
    const on = () => {
      ...
      raf = requestAnimationFrame(() => setY(Math.min(window.scrollY, 600) * factor))
    }
