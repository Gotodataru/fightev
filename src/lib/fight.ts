...
/**
 * The word never claims more than the track record backs. There used to be a fourth
 * step, "clear favourite", from 68% up — but the confident end of the calibration is
 * where the model has done worst (65%+ called 6 of 12), and the step landed first on a
 * newcomer with two UFC bouts. So "edge" is the ceiling; the number still shows how far.
 */
...
/**
 * Which side the interface highlights, and the only place that decides it.
 * An even fight highlights neither: the bar and the percentages must never name a
 * winner that the verdict line refuses to name.
 */
export function leads(p1: number, side: 1 | 2): boolean {
  if (verdict(p1) === 'even') return false
  const [a, b] = split(p1)
...
/**
 * How far apart two values have to be before the tape calls one of them better.
 *
 * The verdict line already refuses to name a winner inside four points of even; the
 * comparison rows had no such rule, so any difference at all was painted as a win —
 * 165 cm of reach against 170 read exactly like 0.82 takedowns against 2.79, and that
 * happens across ten rows of every fight. Below this the numbers still stand side by
 * side, and neither is marked.
 */
const EVEN_ROW = 0.05
...
export function tapeRows(f1: Fighter, f2: Fighter, t: Dict, lang: 'ru' | 'en'): TapeRow[] {
  ...
  return spec.map(([key, field, better, fmt]) => {
    ...
    let adv: 0 | 1 | 2 = 0
    if (better && numeric && v1 !== v2) {
      const scale = Math.max(Math.abs(v1), Math.abs(v2))
      const apart = scale > 0 && Math.abs(v1 - v2) / scale >= EVEN_ROW
      if (apart) adv = (v1 > v2) === (better === 'hi') ? 1 : 2
    }
