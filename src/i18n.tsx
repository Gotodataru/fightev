...
export type Verdict = 'even' | 'slight' | 'edge'
...
const ru = {
  ...
  // list
  card: 'Кард',
  heroLead: 'Прогноз ML-модели на каждый бой, статистика бойцов и честная история её ошибок.',
  independent: 'Независимый проект, с UFC не связан',
  statFights: 'боёв в карде',
  statChecked: 'прогнозов сверено',
  statAccuracy: 'точность модели',
  statVsMarket: (p: number) => `рынок — ${p}%`,
  ...
  finishModelNote: 'Это отдельная модель досрочки. На проверенных боях она не обошла простое правило «всегда досрочно» — сверяйтесь со страницей точности.',
  ...
  notFoundCta: 'К ближайшему турниру',
  ...
  footer: {
    ...
    photos: 'Фотографии бойцов принадлежат правообладателям и здесь не публикуются — вместо них именная плашка.',
    accuracy: 'Точность модели и выборка →',
  },
}

const verdictRu: Record<Verdict, string> = {
  even: 'Равный бой', slight: 'Небольшой перевес', edge: 'Перевес',
}
const verdictEn: Record<Verdict, string> = {
  even: 'Even fight', slight: 'Slight edge', edge: 'Edge',
}
...
const en: Dict = {
  ...
  card: 'Fight card',
  heroLead: 'An ML forecast for every fight, the fighters’ stats, and an honest record of the model’s misses.',
  independent: 'An independent project, not affiliated with UFC',
  statFights: 'fights on the card',
  statChecked: 'forecasts checked',
  statAccuracy: 'model accuracy',
  statVsMarket: (p: number) => `market — ${p}%`,
  ...
  finishModelNote: 'This is the separate finish model. On checked fights it did not beat a simple “always a finish” rule — see the accuracy page.',
  ...
  notFoundText: 'The link may be out of date, or the address has a typo.',
  notFoundCta: 'Go to the next event',
  ...
  footer: {
    ...
    photos: 'Fighter photographs belong to their rights holders and are not published here — a nameplate stands in.',
