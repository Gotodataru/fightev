...
import { Z600, cx, leads, lowData, record, split, verdict } from '../lib/fight'
import { PHOTOS_PUBLISHED } from '../data'
  ...
  return (
    <div className="flex flex-col gap-[7px]">
      <div className="flex items-baseline justify-between">
        <Pct v={a} fav={leads(fight.p_win_f1, 1)} />
        <span className="eyebrow text-zinc-600">{t.win}</span>
        <Pct v={b} fav={leads(fight.p_win_f1, 2)} />
      </div>
...
/** Which nameplate is set as the favourite — by the same rule as the bar, so an even
 *  fight names nobody. */
const plateFav = (fight: Fight, side: 1 | 2): boolean | null =>
  verdict(fight.p_win_f1) === 'even' ? null : leads(fight.p_win_f1, side)
    ...
    <>
      <div className="grid grid-cols-[200px_minmax(0,1fr)_200px] gap-9 px-5 py-6">
        <div className="flex flex-col gap-[18px]">
          <Portrait f={fight.fighter_1} fav={plateFav(fight, 1)} />
          <SidePanel f={fight.fighter_1} />
        </div>
        <div className="stack flex flex-col gap-2.5 pt-1">
          <PredictionBlock fight={fight} track={track} />
          <Module i={3} title={t.tape}><TapeDesktop fight={fight} /></Module>
        </div>
        <div className="flex flex-col items-end gap-[18px]">
          <Portrait f={fight.fighter_2} fav={plateFav(fight, 2)} align="end" />
...
/**
 * React hands out ids like ":r7:". They are legal HTML ids, but a colon starts a
 * pseudo-class in a selector, so `document.getElementById` finds them while
 * `querySelector('#' + id)` throws and `#\:r7\:` is the only way to write them in CSS.
 * The ids here are the ones aria-controls and aria-labelledby point at, and anything
 * reading this page — a test, a stylesheet, an extension — should be able to address
 * them the ordinary way.
 */
const useDomId = () => useId().replace(/[^A-Za-z0-9_-]/g, '')
...
function Tabs({ fight, track, desktopRow }: { fight: Fight; track: Track; desktopRow: boolean }) {
  ...
  // The forecast opens first. The row you tap does show the percentages, but opening a
  // fight folds that row into its header — so with the tape first, the forecast left the
  // screen at the moment you asked to see more of it, and came back one tap later.
    ...
    <div className="flex flex-col gap-5 px-3.5 pb-[18px] pt-4 md:px-5">
      ...
      {PHOTOS_PUBLISHED && (
        <div className="grid grid-cols-2 gap-5">
          <div><Portrait f={fight.fighter_1} fav={plateFav(fight, 1)} w={180} /></div>
          <div className="flex justify-end"><Portrait f={fight.fighter_2} fav={plateFav(fight, 2)} w={180} align="end" /></div>
        </div>
      )}
        ...
        <div className={cx('flex min-h-11 flex-col gap-3 px-3.5 pb-3 pt-3.5', !solo && 'fight-row')} onClick={toggle}>
          ...
          {open ? (
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2.5">
                <Pct v={split(fight.p_win_f1)[0]} fav={leads(fight.p_win_f1, 1)} />
                <div className="flex-1"><ProbBar p1={fight.p_win_f1} animate delay={0.15} /></div>
                <Pct v={split(fight.p_win_f1)[1]} fav={leads(fight.p_win_f1, 2)} />
              </div>
