    ...
    <div className="relative flex min-h-screen flex-col bg-bg">
      ...
      <header className={cx('site-header border-b border-hair', scrolled && 'scrolled')}>
        <div className="mx-auto flex h-[52px] max-w-[1024px] items-center justify-between px-4 md:h-[58px] md:px-6">
          ...
          {/* the menu is only in the DOM while it is open, so aria-controls may only point
              at it then: an attribute naming a missing element is worse than no attribute,
              and aria-expanded carries the state either way */}
          <button type="button" className="-mr-2 flex h-11 w-11 items-center justify-center text-zinc-400 md:hidden"
            aria-expanded={menu} aria-controls={menu ? 'mobile-menu' : undefined} aria-label={menu ? t.nav.close : t.nav.menu}
            onClick={() => setMenu(m => !m)}>
