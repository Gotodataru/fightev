...
import {
  ...
  lastName, leads, lowData, split, tapeRows, verdict,
} from '../lib/fight'
  ...
  return (
    <div>
      <div className="grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-3">
        <Pct v={a} fav={leads(p1, 1)} size={16} />
        <ProbBar p1={p1} height={8} animate />
        <Pct v={b} fav={leads(p1, 2)} size={16} className="text-right" />
      </div>
      ...
      <p className="m-0 mt-2 text-xs leading-normal text-zinc-500">
        ...
        <span className={leads(p1, 1) ? 'font-semibold text-zinc-50' : undefined}>{lastName(f1.name)} {a}%</span>
        {', '}
        <span className={leads(p1, 2) ? 'font-semibold text-zinc-50' : undefined}>{lastName(f2.name)} {b}%</span>
      </p>
    </div>
  )
}
  ...
  return (
    <div role="table" aria-label={t.tape}>
      {tapeRows(fight.fighter_1, fight.fighter_2, t, lang).map((r, k) => {
        ...
        return (
          // a row may own only cells. The two bars are a second drawing of the numbers
          // either side of them, so they stay out of the accessibility tree — and so does
          // the spacer that holds their column when a row has no bars. Without that the
          // row owned a bare div and the table stopped being a valid table.
...
export function SidePanel({ f, align = 'start' }: { f: Fighter; align?: 'start' | 'end' }) {
  ...
  return (
    <div className={cx('flex flex-col gap-[18px]', right && 'text-right')}>
      ...
      <div>
        <SectionLabel end={right} whose={f.name}>{t.howWins}</SectionLabel>
        {tot ? (
          <>
      ...
      <div>
        <SectionLabel end={right} whose={f.name}>{t.zones}</SectionLabel>
      ...
      <div>
        <SectionLabel end={right} whose={f.name}>{t.last5}</SectionLabel>
